...
export function LabDashboard({ t, onNavigate, onViewDetail }) {
  const { activeStats, activeSamples } = useLab();
  const C = useLabTheme();
  ...
  const HealthBar = ({ value }) => {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        <div style={{ width: '5rem', height: '0.4rem', background: C.trendBg, borderRadius: '9999px', overflow: 'hidden' }}>
        ...
        {stats.map((stat, i) => {
          ...
          return (
            <motion.div key={i}
              ...
              style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: '0.75rem', padding: '1.25rem', display: 'flex', flexDirection: 'column', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.875rem' }}>
                <div style={{ background: stat.bg, width: '2.5rem', height: '2.5rem', borderRadius: '0.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white' }}>
                  <Icon size={18} />
                </div>
                <span style={{ fontSize: '0.7rem', fontWeight: 700, padding: '0.2rem 0.5rem', borderRadius: '0.375rem', background: C.bgAlt, color: stat.trendDown ? C.textMuted : '#10b981' }}>
                  {stat.trend}
                </span>
              </div>
              <div style={{ fontSize: '1.75rem', fontWeight: 900, color: C.text, lineHeight: 1, marginBottom: '0.25rem' }}>{stat.value}</div>
              <div style={{ fontSize: '0.8125rem', color: C.textMuted, fontWeight: 500 }}>{stat.title}</div>
            </motion.div>
          );
        })}
      </div>
