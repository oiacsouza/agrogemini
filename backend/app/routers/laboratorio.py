...
@router.get("/")
async def list_laboratorios(db=Depends(get_db_connection), user=Depends(get_current_user)):
...
@router.get("/{lab_id}")
async def get_laboratorio(lab_id: int, db=Depends(get_db_connection), _=Depends(get_current_user)):
    return LaboratorioService(db).get_by_id(lab_id)


@router.post("/", status_code=201)
async def create_laboratorio(data: LaboratorioCreate, db=Depends(get_db_connection), _=Depends(get_current_user)):
    return LaboratorioService(db).create(data)


@router.put("/{lab_id}")
async def update_laboratorio(lab_id: int, data: LaboratorioUpdate, db=Depends(get_db_connection), _=Depends(get_current_user)):
    return LaboratorioService(db).update(lab_id, data)


@router.delete("/{lab_id}")
async def delete_laboratorio(lab_id: int, db=Depends(get_db_connection), _=Depends(get_current_user)):
...
async def list_lab_usuarios(lab_id: int, db=Depends(get_db_connection), _=Depends(get_current_user)):
    return LaboratorioService(db).get_usuarios(lab_id)


@router.post("/{lab_id}/usuarios", status_code=201)
async def add_lab_usuario(lab_id: int, data: LabUsuarioCreate, db=Depends(get_db_connection), _=Depends(get_current_user)):
...
async def remove_lab_usuario(lab_id: int, assoc_id: int, db=Depends(get_db_connection), _=Depends(get_current_user)):
...
async def list_lab_telefones(lab_id: int, db=Depends(get_db_connection), _=Depends(get_current_user)):
    return LaboratorioService(db).get_telefones(lab_id)


@router.post("/{lab_id}/telefones", status_code=201)
async def add_lab_telefone(lab_id: int, numero: str, tipo: str = "COMERCIAL", db=Depends(get_db_connection), _=Depends(get_current_user)):
