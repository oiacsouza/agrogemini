...
async def resync_all_sequences(session: AsyncSession):
    logger.info("Resyncing Oracle Identity sequences...")
        ...
        if 'id' in table.columns:
            ...
            if hasattr(col, 'identity') and col.identity is not None:
                try:
                    ...
                    logger.warning(f"Não foi possível resincronizar a sequência de {table_name}: {e}")
                    
    # Drop legacy check constraints that conflict with V7 seed data
    legacy_constraints = [
        ("planos_assinaturas", "CK_PLANOS_TIPO"),
        ("usuarios", "CK_USUARIOS_PLANO")
    ]
    for table_name, constraint_name in legacy_constraints:
        try:
            await session.execute(text(f"ALTER TABLE {table_name} DROP CONSTRAINT {constraint_name}"))
            logger.info(f"Constraint {constraint_name} removida de {table_name}.")
        except Exception as e:
            # Oracle error ORA-02443: Cannot drop constraint - nonexistent constraint
            pass
            
    # Fix column length that might be too small from older schemas
    try:
        await session.execute(text("ALTER TABLE usuarios MODIFY plano_ativo VARCHAR2(50)"))
        logger.info("Tamanho da coluna plano_ativo ajustado para VARCHAR2(50).")
    except Exception as e:
        pass
        
    await session.commit()
    logger.info("Sequences and legacy constraints adjusted.")
...
async def seed_db():
    async with AsyncSessionLocal() as session:
        try:
            ...
            # Resync sequences to avoid ORA-00001
            await resync_all_sequences(session)
            ...
            # 6. USUARIOS ESTRUTURADOS (40)
            existing_users_list = (await session.execute(select(Usuario))).scalars().all()
            existing_by_email = {u.email: u for u in existing_users_list}
            ...
            def get_or_create_user(nome, sobrenome, email, tipo_usuario, plano_ativo):
                if email in existing_by_email:
                    u = existing_by_email[email]
                    u.plano_ativo = plano_ativo
                else:
                    u = Usuario(nome=nome, sobrenome=sobrenome, email=email, senha_hash=pwd, tipo_usuario=tipo_usuario, ativo="Y", plano_ativo=plano_ativo)
                    session.add(u)
            ...
            # ADM (1)
            get_or_create_user("Admin", "Sistema", "admin@agrogemini.com", "ADM", "ADM-PLAN")

            # UP - Donos de Lab (10)
            for i in range(10):
                ups.append(get_or_create_user("Gestor", f"Lab {i+1}", f"gestor{i+1}@agrogemini.com", "UP", "PREMIUM LAB"))

            # UC - Tecnicos de Lab (4)
            for i in range(4):
                ucs.append(get_or_create_user("Tecnico", f"{i+1}", f"tecnico{i+1}@agrogemini.com", "UC", "FUNCIONARIO LAB"))

            # UE - Produtores (5) -> 3 Premium Prod, 2 Free
            for i in range(5):
                plano = "PREMIUM PROD" if i < 3 else "FREE"
                ues.append(get_or_create_user("Produtor", f"{i+1}", f"produtor{i+1}@agrogemini.com", "UE", plano))

            # UL - Usuarios de Laboratorio Institucionais (20)
            for i in range(20):
                plano = "FILIAL LAB" if labs[i].tipo_unidade == "FILIAL" else "PREMIUM LAB"
                uls.append(get_or_create_user("Institucional", labs[i].nome, labs[i].email, "UP", plano))

            await session.flush()
            logger.info("6. Usuários criados ou atualizados com lógica hierárquica (40).")
